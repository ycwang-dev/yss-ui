...
const SearchBar: React.FC = () => {
  ...
  const { keywords, setKeywords, result, loading, load: loadSearchData } = useSiteSearch();
  const [modalVisible, setModalVisible] = useState(false);
  const isHomepage = 'hero' in frontmatter;
  ...
  useEffect(() => {
    ...
    const handler = (ev: KeyboardEvent) => {
      if (
        (((isAppleDevice ? ev.metaKey : ev.ctrlKey) && ev.key === 'k') || ev.key === '/') &&
        !isInput(ev.target as HTMLElement)
      ) {
        ev.preventDefault();
  ...
  return (
    <div
      className={`dumi-default-search-bar${isHomepage ? ' search-bar-demo-active' : ''}`}
      ref={wrapperRef}
      onMouseMove={handleMouseMove}
    >
      <div className="yss-search-bar-glow-layer"></div>
