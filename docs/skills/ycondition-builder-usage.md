...
> 指导规则配置、数据质量条件、嵌套 AND/OR 条件树使用 @yss-ui/components 的 YConditionBuilder；覆盖 ConditionGroup/ConditionLeaf、loadFields/loadValues、getOperators、BETWEEN/IN、strictMode 校验和 disabled，禁止把 readonly 或未消费的 segments 当成可用 API。
